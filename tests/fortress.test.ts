...
describe('Semantic Model Keep', () => {
  ...
  it('the Card is out-stared in one look and the pie, made a bar chart, drops the boots', () => {
    const { s, outs } = play(['look at card', 'use pie chart', 'wear boots'], 'fortress.yard', {});
    expect(outs[0]![0]).toMatch(/blinks/i);
    expect(s.flags['stare.done']).toBe(true);
    expect(s.flags['stare.count']).toBe(3);
    expect(s.flags['refresh.done']).toBe(true);
    expect(s.worn).toContain('boots');
    expect(s.score).toBe(25); // the stare (+10) and the pie (+15)
    // The policy is a nudge now: no points, no boots, and it stays in your pocket.
    const t = { ...newGame(WORLD, 6), room: 'fortress.yard', inventory: ['license', 'policy'] };
    const r = step(t, 'use policy on refresh', WORLD);
    expect(r.stepId).toBe('fortress.policy-nudge');

...
export const NPCS: Record<string, Npc> = Object.fromEntries([
  npc({
    ...
    talkMore: (s, n) => {
      ...
      if (done && calm) return jeffHears(s, n === 2 ? '"Told no one," Jeff says again. "Telling you doesn\'t count. You were there."'
        : n === 3 ? '"The dragon is east; I read the board. Now I would like to be left alone with my mug."'
        : cycle(n, [`"Lookin' good, ${nick(s)}. The dragon's east."`, `"Nothing left in me, ${nick(s)}. Go be Worthy."`, `"I'm content, ${nick(s)}. It's unsettling for both of us."`]));
      if (done) return jeffHears(s, n === 2 ? '"The report was right," Jeff says, quieter, looking at his empty hand.'
        : n === 3 ? '"There\'s a mug on your desk, west. I\'d tell it the report was right."'
        : cycle(n, [`"Still no mug, ${nick(s)}. I was right, and I have nothing to drink out of."`, `"Cottage. Desk. Mug. Me, ${nick(s)}."`, `"I can be right AND thirsty, ${nick(s)}. I'm doing it now."`]));
      ...
      if (calm && !setting(s, 'export')) return jeffHears(s, n === 2 ? '"The numbers, though," Jeff says into his mug. "They don\'t match. I have Excel open."'
        ...
        : cycle(n, [`"Lookin' good, ${nick(s)}. The spreadsheet can wait; it has to."`, `"Two words would fix this, ${nick(s)}. Neither of them is mine to say."`, `"I've got the mug and the numbers, ${nick(s)}. I've got nowhere to open them."`]));
      if (calm) return jeffHears(s, n === 2 ? '"The numbers, though," Jeff says into his mug. "They don\'t match. I have Excel open." (Say: help jeff.)'
        : n === 3 ? '"Say help jeff and come look at Excel. I\'ll do the clicking; no I won\'t."'
        : cycle(n, [`"Lookin' good, ${nick(s)}. Now the spreadsheet."`, `"help jeff, ${nick(s)}. It's a spreadsheet, not a schema."`, `"I've got the mug and the numbers, ${nick(s)}. help jeff."`]));
      return jeffHears(s, n === 2 ? '"Export? Excel? Either. Both. I\'m flexible," says Jeff, who is not.'
        : n === 3 ? '"That mug on your desk would help. West; I can see it through your window."'
        : cycle(n, [`"Still no mug, ${nick(s)}."`, `"The mug's in the cottage, west, ${nick(s)}. I've drawn you a map on my spreadsheet."`, `"I could just take the export, ${nick(s)}. I won't. Mug."`]));
    },
  ...
  npc({
    ...
    talkMore: (s, n) => (s.flags['ferry.online']
      ? cycle(n + 2, [`"Heard the lamp came on. Now go away, ${nick(s)}; I'm being decommissioned."`, `"Lookin' good, ${nick(s)}. Gen1 lives on, in a boat."`, `"The Mill has nothing left for you, ${nick(s)}. It barely had that."`])
      : n === 2 ? '"Q3," says the Miller, shorter. "Take care of those creds."'
      : n === 3 ? '"Those creds are the Ferryman\'s. South to the square, south to the lake, east to the dock."'
      : cycle(n, [`"Still carrying my creds, ${nick(s)}. The dock's that way."`, `"Square, lake, dock, ${nick(s)}. Give them to him; don't say them."`, `"They're a scrap of parchment, ${nick(s)}. They're not going to walk to the dock themselves."`])),
  ...
  npc({
    ...
    talkMore: (s, n) => {
      if (s.flags['ferry.online']) return n === 2 ? '"ONLINE," he says, still tasting it. "Board when ready."'
        : n === 3 ? '"Board the boat. On the Isle, take the STANDARD key; Personal is for people who close their laptops."'
        : cycle(n, [`"Lookin' good, ${nick(s)}. Board."`, `"The boat's right there, ${nick(s)}. It says GATEWAY on it."`, `He hums the boarding call at you, ${nick(s)}. It's always the boarding call.`]);
      if (n === 2) return '"Credentials," he mouths again, slower, the way you mouth a word to someone across a very wide lake. "Expired."';
      if (n === 3) return '"The Mill has Gen1 credentials," he mouths. "West, north, north. Ask the Miller, or open his chest."';
      return s.inventory.includes('credentials')
        ? cycle(n, [`He looks at your pocket, ${nick(s)}. The creds are RIGHT THERE.`, `"Give. Credentials. To. Me," he mouths, pointing at himself, ${nick(s)}.`, `You have the creds and he has the lamp, ${nick(s)}. Introduce them.`])
  ...
  npc({
    ...
    talkMore: (s, n) => (s.flags['gate.open']
      ? n === 2 ? 'The monk points at the open gate, then at you. Some things do not need a session.'
        : n === 3 ? '"North," says the monk. "The Abbot is in the cloister; go before it times out."'
        : cycle(n, [`The monk has said his word, ${nick(s)}. North.`, `Lookin' good, ${nick(s)}. Go in before the session times out.`, `He points north again, ${nick(s)}, with both hands this time.`])
      : n === 2 ? 'The monk points at the progress bar again, a little harder. It is still starting.'
        : n === 3 ? '"Open it, or wait," the monk says, quietly, so the session doesn\'t hear. "Either one counts."'
        : cycle(n, [`Still not opening it, ${nick(s)}. The bar noticed.`, `"OPEN," the monk mouths at you, ${nick(s)}. He has broken his vow for this.`, `A hand on the gate, or a wait, ${nick(s)}. Talking is neither.`])),
  ...
  npc({
    ...
    talkMore: (s, n) => {
      if (s.flags['has.hoodie']) return n === 2 ? '"Notebooks," the Abbot says again. "Never look back, as I am doing now."'
        : n === 3 ? (xmlaOff(s)
          ? '"The back gate is a wall, child. XMLA endpoint: Read Write is off, in the Capacity Ledger upstairs; turn it on, or go west from the gate through the Gold Marsh, the long way, to the Peaks."'
          : '"South, child, through the Keep, to the Peaks. The Worthy Three are worn, smelt, and held; you wear one."')
        : cycle(n, [`"The hood suits you, ${nick(s)}. It suits everyone; that is the problem with hoods."`, `"Go, ${nick(s)}. The dragon is east of everything."`, `"You are still here, ${nick(s)}. The hoodie is not a chair."`]);
      ...
      if (notebookBlockers(s).length) {
        if (n === 3) return `"No Notebook until ${notebookBlockerText(s)} back on; the Sacristy, up the stair, has it. Then the scroll, west; then the notebook, east; then kneel."`;
        return s.inventory.includes('scroll')
          ? cycle(n, [`"You have the scroll, ${nick(s)}. The notebook needs the Sacristy first; up the stair."`, `"The book first, ${nick(s)}. Then the notebook."`, `"Still holding the scroll, ${nick(s)}? It reads better on a notebook you are allowed to have."`])
          : cycle(n, [`"Still pandas, ${nick(s)}. Still no Fabric items."`, `"Up for the book, west for the scroll, east for the notebook, ${nick(s)}."`, `"The Hoodie of Spark waits, ${nick(s)}. The Sacristy is a stair away."`]);
      }
      if (n === 3) return '"The scroll is in the Library, west; your license opens the case. Then east, east, and use the scroll on the notebook."';
      return s.inventory.includes('scroll')
        ? cycle(n, [`"You have the scroll, ${nick(s)}. Use it on the notebook, east."`, `"The notebook is one room east and forty-one minutes deep, ${nick(s)}."`, `"Still holding the scroll, ${nick(s)}? It reads better on a notebook."`])
        : cycle(n, [`"Still pandas, ${nick(s)}."`, `"West for the scroll, east for the notebook, ${nick(s)}."`, `"The Hoodie of Spark waits, ${nick(s)}. It has waited through worse."`]);
    },
  ...
  npc({
    ...
    talkMore: (s, n) => (noNotebook(s)
      ? n === 2 ? '"Who did this," Brother Pandas says again, looking at you. He has been looking at you the whole time.'
        : n === 3 ? ({
          shelf: '"In the Sacristy, up the stair from the Cloister, a book on the shelf says Users can create Fabric items, and somebody shut it. Open it again and I can stop being a report."',
          ledger: '"In the Sacristy, up the stair from the Cloister, the Capacity Ledger has Delegated tenant settings shut. Turn it back on and I can stop being a report."',
          both: '"In the Sacristy, up the stair from the Cloister: Users can create Fabric items is shut on the shelf, and Delegated is shut in the Capacity Ledger. Open both; I will wait."',
        })[pandasSwitch(s)]
        : cycle(n, [`"Still no Notebook, ${nick(s)}. I am still a report."`, `"The Sacristy, ${nick(s)}. ${({ shelf: 'The shelf. Fabric items. On.', ledger: 'The Ledger. Delegated. On.', both: 'The shelf AND the Ledger. On, and on.' })[pandasSwitch(s)]}"`, `"I can make a dashboard, ${nick(s)}. Nobody wants a dashboard, not even the dashboard."`])
      : s.flags['notebook.fixed']
      ? n === 2 ? '"Four seconds," Brother Pandas says again. He has said it forty times.'
        : n === 3 ? '"The Abbot wants you, west. He has been holding a hoodie at arm\'s length."'
        : cycle(n, [`"Lookin' good, ${nick(s)}. Spark suits you."`, `"Delta," he whispers at you, ${nick(s)}. He just likes saying it now.`, `"It works in production, ${nick(s)}. I don't know what to do with my hands."`])
      : n === 2 ? '"It works on my laptop," Brother Pandas repeats. "My laptop is not here."'
        : n === 3 ? '"The cell says pd.read_csv; it wants spark.read. The Library, west, west, has a scroll that says so."'
        : cycle(n, [`"Still running, ${nick(s)}. Forty-one minutes. Forty-two."`, `"The scroll, on the notebook, ${nick(s)}. I'd do it, but I'd do it in pandas."`, `"I could restart the session, ${nick(s)}. That's four more minutes."`])),
    ...
    talkMore: (s, n) => (s.flags['scroll.lent']
      ? n === 2 ? '"By the end of the session," she says again, and the session, somewhere, starts.'
        : n === 3 ? (notebookBlockers(s).length
          ? `"East, then east, the notebook; it will not take the scroll until ${notebookBlockerText(s)} back on, and the Sacristy, up the stair, has it. And bring the scroll back."`
          : '"East, then east: use the scroll on the notebook. And bring it back, which nobody ever has."')
        : cycle(n, [`"Shh, ${nick(s)}."`, `"Lookin' good, ${nick(s)}. Now read it; nobody reads them."`, `"You have the scroll and I have your card, ${nick(s)}. That's a transaction."`])
      : n === 2 ? '"Library card," she says again. "Yours. The one in your pocket, which you keep touching."'
        : n === 3 ? '"Give me your license: give license to librarian. I will keep it as collateral."'
        : cycle(n, [`"Still no card, ${nick(s)}. It's in your inventory; I can see it from here."`, `"Give. License. Librarian, ${nick(s)}. Three words; I'll allow it."`, `"The scroll is not going to lend itself, ${nick(s)}."`])),
  ...
  npc({
    ...
    talkMore: (s, n) => (s.flags['bridge.down']
      ? n === 2 ? '"Sixty days," the guard repeats, slower, holding up six fingers and then, after some thought, a zero.'
        : n === 3 ? '"Sixty. Days. Go in. The hall is north; the Duke is north of that."'
        : cycle(n, ["'Lookin' good, Mr. Trial.'", `'Still here, ${nick(s)}? The Keep is north, where the drawbridge fell.'`, `'You have a trial and a Keep, ${nick(s)}. Use one on the other.'`])
      : n === 2 ? '"HALT," the guard says again, quieter, as if halting were something you could do more of. "Your SKU."'
        ...
        : cycle(n, [`'Still no SKU, ${nick(s)}.'`, `'Almost there, ${nick(s)}, but that's PRO.'`, `'Say it or start it, ${nick(s)}: T-R-I-A-L. It's free.'`])),
  ...
  npc({
    ...
    describe: () => 'The Duke of DAX, on a throne carved as a formula bar. He speaks only in filter context. He has never once used SQL and would like that in writing.',
    ...
    talkMore: (s, n) => (s.flags['trial.moat']
      ? n === 2 ? 'The Duke pretends harder not to see you. He learned it in a design review.'
      ...
      : n === 2 ? "'CALCULATE(,' says the Duke, slower, and waits. You have brought nothing to put in the parentheses."
        : n === 3 ? "'Say the one thing I will not hear in this chamber,' says the Duke. 'Two words. It goes in a table.'"
        : cycle(n, [`'Still no sin, ${nick(s)}. Say the two words, or hand me a table I can despise.'`, `'Not SQL, ${nick(s)}. A modeling shortcut; the lazy one.'`, `'Think smaller than SQL, ${nick(s)}. Think of a column.'`])),
  ...
  npc({
    ...
    talkMore: (s, n) => (n === 2 ? CARDINALITY_ADVICE[1]!
      : n === 3 ? (xmlaOff(s)
        ? `${CARDINALITY_ADVICE[2]} He looks toward the north gate, which is bricks. 'XMLA endpoint: Read Write is off; the Capacity Ledger in the monks' Sacristy has it, or go round by the Gold Marsh. The Duke is east, then north.'`
        : `${CARDINALITY_ADVICE[2]} He looks toward the north gate. 'The monks are that way. The Duke is east, then north; take him the date table.'`)
      : cycle(n, [`"One. To. Many, ${nick(s)}."`, `"Star schema, ${nick(s)}. I will say it at your funeral."`, `"The dragon wants two words, ${nick(s)}. I have said them."`])),
  ...
  npc({
    ...
    talkMore: (s, n) => (s.flags['stare.done']
      ? n === 2 ? 'The Card says 4.2M again, slightly louder. It has one value and it is committed to it.'
        : n === 3 ? (s.flags['refresh.done'] ? 'The Card shows 4.2M and the refresh is done. Nothing left here to stare at.' : 'The Big Refresh is stuck at 97% on a pie with thirty-one slices. The problem is round.')
      ...
      : n === 2 ? '(Blank). Again. The Card is not stalling; it has nothing to stall with.'
        : n === 3 ? 'Look at the Card and do not blink; it will show a number. Blink and it wins.'
        : cycle(n, [`Still (Blank), ${nick(s)}.`, `Look at it, ${nick(s)}. Just look.`, `(Blank) is not a conversation, ${nick(s)}. It is a measure that never got written.`])),
  ...
  npc({
    ...
    talkMore: (s, n) => (n === 2 ? '"WHAT IS THE ONE TRUE MODEL," Throttlor repeats, slower, with more smoke. He is billing you for the repeat.'
      : n === 3 ? '"Two words, peasant. One fact table, some dimensions, a shape; Sir Cardinality said it, and I heard him from here."'
      : cycle(n, [`"Still no answer, ${nick(s)}. The question has not changed, and neither has the fee."`, `"It's a schema, ${nick(s)}, shaped like a thing in the sky."`, `Throttlor yawns fire at you, ${nick(s)}. "Answer, or I start smoothing you over 24 hours."`])),
  ...
  npc({
    ...
    // A second look in a row gets the short one.
    describe: (s) => ((s.recent?.n ?? 1) >= 2
      ? 'Still straw. Still Monday.'
      : 'A scarecrow named Manual, who has to be triggered by hand. Around his neck hangs a sign: MONDAY. It is not a surname; it is a warning about full refreshes.'),
    talk: () => 'Manual says nothing. He is waiting for someone to click him.',
    talkMore: (s, n) => (n === 2 ? 'Manual says nothing, again. He is very consistent, for a scarecrow.'
      : n === 3 ? 'You could use him; Manual refreshes when clicked. That is his whole thing, and his whole tragedy.'
      : cycle(n, [`Manual, ${nick(s)}, is a scarecrow. He is not going to open up.`, `Nothing, ${nick(s)}. Not even a schedule.`, `Manual waits, ${nick(s)}. He is good at it.`])),

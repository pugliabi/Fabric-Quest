...
export const SETTINGS: Setting[] = [
  { key: 'export', shelf: 'tenant', title: 'Export to Excel', words: ['export', 'excel', 'analyze', 'export data', 'export to excel', 'users can export data'],
    read: "Lets anyone export the data behind a visual to an Excel file. Every Jeff in the realm depends on it. Here, that means: Jeff's Excel opens while it's on, and stays locked when it's off.",
  ...
  { key: 'publishToWeb', shelf: 'tenant', title: 'Publish to web', words: ['publish to web', 'web', 'publish', 'internet publishing'],
    ...
    on: 'Publish to web is on. 4,112 people can read the prophecy again. One of them is a dragon.', off: 'Publish to web is off. The prophecy is private again. The embed codes go dark, one by one.', effect: 'On: the notice board has views, and My Workspace offers the entire internet.' },
  { key: 'guests', shelf: 'tenant', title: 'Guest users can access Microsoft Fabric', words: ['guest', 'guests', 'guest users', 'b2b'],
    read: 'Lets B2B guests, invited from other tenants, into your Fabric. Here, that means: with Publish apps to the entire organization also on, the whole organization comes to the Square.',
  ...
  { key: 'surge', shelf: 'capacity', title: 'Surge protection', words: ['surge', 'surge protection', 'protection'],
    ...
    on: 'Surge protection is on. Somewhere on the Peaks, a dragon puts on a hard hat.', off: 'Surge protection is off. The hard hat comes off. The dragon looks exactly the same.', effect: 'On: the dragon has a hat. That is all.' },
...
export const SACRISTY_PHRASES: PhraseRule[] = [
  ...
  { id: 'sacristy.stair-up', room: 'monastery.cloister', test: STAIR, text: '', then: (_s, _w, line) => ({ then: { text: /\b(down|downstairs|descend)\b/.test(line) ? 'The stair goes up from here. Down is the floor, and the monks are using it. You climb.' : 'You climb the spiral stair. Every step is worn in the middle by an admin who went up to change one thing.', moveTo: SACRISTY } }) },
...
export const SACRISTY_ROOM: Room = {
  ...
  rules: [
    ...
    { id: 'sacristy.talk-book', when: { verb: 'talk', nounMatches: BOOK_WORDS }, then: { text: 'The book does not talk. It has a switch, not a mouth.', outcome: 'fail' } },
    { id: 'sacristy.say-dax', when: { verb: 'say', noun: ['dax'] }, then: { text: "You say 'DAX' in the Admin Portal. Nothing here is a measure. It is all switches.", outcome: 'snark' } },
  ],

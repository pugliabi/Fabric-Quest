...
const ticketTalk = (s: GameState): string => (s.flags['hall.ticket']
  ? "'Who's working my ticket?' you ask. 'It's In Progress,' says the Clerk. 'Who, though?' 'That's an admin setting.' 'Is it you?' 'In Progress.'"
  : "'That ticket's on the counter,' says the Clerk. 'It's In Progress.' You haven't filed it. 'That's an admin setting.'");
const ticketStatus = (s: GameState): string => (s.flags['hall.ticket']
  ? again(s, 'You check your ticket. In Progress. You refresh. In Progress, with a newer timestamp. That is what progress looks like.', `You check again. It has been ${MALAPROPS.refreshered} twice now. In Progress. The timestamp is the only thing moving.`)
...
const LATER = [
  ...
  'Admin setting. Next.',
  '…the admins are in the Sacristy. Up the stair from the Cloister. I said that already; it is also an admin setting.',
] as const;
...
export const HALL_ROOM: Room = {
  ...
  rules: [
    ...
    { id: 'hall.give-ticket-counter', when: { verb: 'give', noun: TICKET, noun2: CLERK, notHas: ['ticket'], flags: [{ flag: 'taken.ticket', not: true }] }, then: { text: "The ticket is on the counter, which is the Clerk's side. Pick it up first. Then it is your problem, and you can make it his.", outcome: 'fail' } },
    ...
    { id: 'hall.counter', when: { verb: 'use', noun: ['counter', 'desk', 'front desk'] }, then: { text: `You lean on the counter. The Clerk slides a form across. It is blank. That is the form. The form has been ${MALAPROPS.refreshered} since you last looked; it is still blank.`, outcome: 'fail' } },
  ],
};
...
export const TOWNHALL_PHRASES: PhraseRule[] = [
  ...
  { id: 'hall.take-number', room: HALL, test: /^(take|get|pull|grab)( a)? number$/, text: (s) => again(s, 'You take a number. It is 1. The Clerk calls 1. It is an admin setting.', 'You take another number. It is also 1. There is one number. You have it twice now, like a duplicate ticket.') },

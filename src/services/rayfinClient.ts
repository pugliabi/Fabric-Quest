...
let client: RayfinClient<ProsQuestSchema> | null = null;
let localDev = false;

export async function initRayfinClient(
  config: RayfinClientConfig
): Promise<RayfinClient<ProsQuestSchema>> {
  if (client) {
  ...
  client = new RayfinClient<ProsQuestSchema>({
...
export function getRayfinClient(): RayfinClient<ProsQuestSchema> {
  if (!client) {
    throw new Error(
